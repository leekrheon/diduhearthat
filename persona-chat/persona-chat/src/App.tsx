...
export default function App() {
  ...
  const handleAdvanceCount = () => {
    const newToday = Math.min(todayCount + 1, 5);
      ...
      <main className="flex-1 relative overflow-hidden">
        {activeView === 'home' && (
          <motion.div key="home" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0">
            <Home count={todayCount} totalCount={totalCount} weekCounts={weekCounts} onStart={() => setActiveView('select')} />
          </motion.div>
        )}
        {activeView === 'select' && (
        ...
        {activeView === 'credit' && (
          <motion.div key="credit" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 z-20">
            <Credit credits={credits} todayCount={todayCount} user={user} />
          </motion.div>
        )}
        {activeView === 'scrap' && (
        ...
        {activeView === 'profile' && (
          <motion.div key="profile" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 z-20">
            <Profile totalCount={totalCount} credits={credits} user={user} onLogout={handleLogout} />
          </motion.div>
        )}
        <AnimatePresence>
