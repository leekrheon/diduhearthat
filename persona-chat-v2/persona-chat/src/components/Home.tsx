  ...
  return (
    <div className="flex flex-col h-full bg-black text-white p-6 overflow-y-auto pb-24">
      ...
      <div className="flex flex-col items-center justify-center py-10 bg-white/5 rounded-[3rem] mb-6 border border-white/10 shadow-2xl">
        <RingProgress current={count} total={10} size={200} strokeWidth={24} />
        <div className="mt-8 text-center">
          ...
          <p className="text-5xl font-black tracking-tighter text-white">
            {count} <span className="text-xl text-white/30 font-medium">/ 10</span>
          </p>
          <p className="text-white/20 text-xs mt-2 font-medium">
            {count >= 10 ? '🎉 오늘 목표 달성!' : `${10 - count}개 남았어요`}
          </p>
        </div>
      </div>
      ...
      <div
        onClick={onStart}
        className="p-5 bg-white rounded-3xl flex items-center justify-between shadow-2xl shadow-white/5 active:scale-[0.98] transition-transform cursor-pointer"
      >
        <div className="flex items-center space-x-3 text-black">
          <div className="bg-black p-2 rounded-xl">
            <MessageCircle size={18} fill="white" className="text-white" />
          </div>
          <span className="font-bold text-sm">지금 바로 대화하기</span>
        </div>
